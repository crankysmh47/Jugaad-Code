...
Immediately checkpoint all uncommitted work in the current project.

1. Run `git status --porcelain` to check if repo is dirty.
2. If dirty:
   - `git add -A`
   - `git commit -m "chore: [PK-checkpoint] manual @ <current date and time>"`
     (generate the timestamp with the shell's date command — `$(date '+%Y-%m-%d %H:%M')`
     in bash, `Get-Date -Format 'yyyy-MM-dd HH:mm'` in PowerShell)
